...
pub unsafe extern "C" fn sapp_x11_get_screen_size(width: *mut libc::c_int, height: *mut libc::c_int) {
    let mut attribs = XWindowAttributes {
        x: 0,
        y: 0,
        width: 0,
    ...
    XGetWindowAttributes(_sapp_x11_display, _sapp_x11_window, &mut attribs);
    *width = (*attribs.screen).width;
    *height = (*attribs.screen).height;
}
...
pub unsafe fn sapp_set_window_size(new_width: u32, new_height: u32) {
    XResizeWindow(
        _sapp_x11_display,
        _sapp_x11_window,
        new_width as libc::c_uint,
        new_height as libc::c_uint,
    );
}
