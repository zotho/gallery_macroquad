...
pub fn display_size() -> (f32, f32) {
    let context = get_context();
    let (w, h) = context.quad_context.get_display_size();
    (
        w as f32 * context.quad_context.dpi_scale(),
        h as f32 * context.quad_context.dpi_scale(),
    )
}
